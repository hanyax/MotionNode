...
pose_name = ['Good Poseture', 'Bad Poseture']
def pca_process(data, data_num, place):
	...
	plt.tight_layout()
	plt.grid()
	fname = 'pca_placement%d_all_step.png'%(place)
...
def pca_process_classification(data, data_num):
	...
	plt.tight_layout()
	plt.grid()
	plt.savefig('binary_classification_place1_step2.png', dpi=300, format='png', bbox_inches='tight')
	# plt.show()
...
def main(place):
	# load data
	# data1 = load_6dim_data('../Data_output_txt_MotionShield/output_to_txt/goodDataLower1.txt')
	# data1 = load_6dim_data('../Data_output_txt_MotionShield/output_to_txt/goodDataUpper1.txt')
	# data2 = load_6dim_data('../Data_output_txt_MotionShield/output_to_txt/badDataLower1.txt')
	# data2 = load_6dim_data('../Data_output_txt_MotionShield/output_to_txt/badDataUpper1.txt')
	...
	for i in range(4):
		step = i+1
		data1 = load_6dim_data('/Users/kevin/Documents/UWEE/EE484-Sensor/data-0604/goodpose-placement%d/1/data%d.txt'%(place,step))
		data2 = load_6dim_data('/Users/kevin/Documents/UWEE/EE484-Sensor/data-0604/badpose-placement%d/data%d.txt'%(place,step))
		if len(goodpose_data)!=0:
			goodpose_data = np.concatenate((goodpose_data,data1))
		else:
			goodpose_data = data1
		if len(badpose_data)!=0:
			badpose_data = np.concatenate((badpose_data,data2))
		else:
			badpose_data = data2

	# time_plot(data1, 'goodDataLower.png')
	# time_plot(data1, 'goodDataUpper.png')
	# time_plot(data2, 'badDataLower.png')
	# time_plot(data2, 'badDataUpper.png')
	all_data = np.concatenate((goodpose_data ,badpose_data))
	data_num = [len(goodpose_data), len(badpose_data)]

    # run pca
	pca_process(all_data, data_num, place)
